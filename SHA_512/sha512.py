...
# Right rotate function for 64-bit words
def rotr(x, n):
    return ((x >> n) | (x << (64 - n))) & 0xFFFFFFFFFFFFFFFF

# Padding
def sha512_pad(message_bytes):
    message = bytearray(message_bytes)
    message_len = len(message) * 8
    message.append(0x80)
    while (len(message) * 8 + 128) % 1024 != 0:
        message.append(0)
    message += struct.pack('>Q', 0)  # high 64 bits (for > 2^64 bits)
...
def sha512(message):
    ...
    for i in range(0, len(message), 128):
        w = list(struct.unpack('>16Q', message[i:i+128])) + [0]*64
        ...
        for j in range(80):
            ...
            ch = (e & f) ^ (~e & g)
            temp1 = (hh + S1 + ch + K[j] + w[j]) & 0xFFFFFFFFFFFFFFFF
    ...
    return ''.join(f'{val:016x}' for val in h)

# Example usage
if __name__ == "__main__":
    msg = input("Enter message for SHA-512 hashing: ")
    result = sha512(msg)
    print(f"\nSHA-512 Hash:\n{result}")
